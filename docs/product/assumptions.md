...
| A-010 | IGDB is compatible with the current private, non-commercial learning mode using local normalized metadata, direct provider-CDN cover references, and no copied image binaries or external ratings | High | Medium | Supported for the explicitly bounded release mode by the [provider spike](../research/game-data-providers-spike.md#66-terms-and-release-mode-boundary) and [ADR-0001](../decisions/0001-reference-igdb-cover-images.md). Provider covers require attribution, a clear source path, allowlisted delivery, and fallback. Public deployment, monetization, copied or stored images, or redistribution must reopen the provider gate | Supported |
